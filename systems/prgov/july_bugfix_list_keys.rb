...
keys.each do |key|
	...
	# if this request did not happen after june 
	if(time.month < 6 and time.year == 2015) 
		skipped += 1
		next # skip requests that 
	else
		# if in june, but prior to june 30 
		if(time.month == 6 and time.day < 30 and time.year == 2015)
			# dont resend anything before may 30	
			skipped += 1
			next	
		end
		if(time.month == 7 and time.day > 13 and time.year == 2015) 
			# If this happend in june AFTER july 13
			skipped += 1
			next
		end 
	end
	...
	if(tx.email.to_s.include? "yahoo.com" or tx.email.to_s.include? "aol.com" or 
	   ...
	   tx.email.to_s.include? "hotmail.com") 
		...
		#puts "Needs Requeuing #{tx.id} - from #{tx.created_at} - (#{tx.email}}" 
		#print "\"#{tx.id}\", \"#{tx.email}\", \"#{tx.created_at}\","  
		puts "\"#{tx.id}\" => [\"#{tx.email}\", \"#{tx.created_at}\"],"  
	end
end
